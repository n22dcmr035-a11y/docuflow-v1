...
interface PdfAnnotation { id: string; text: string; note: string; color: string; pageNum: number; rects: HRect[]; }
          ...
          >
            {Array.from({ length: numPages }, (_, i) => i + 1).map(pageNum => {
              ...
              return (
                ...
                >
                  {/* Page canvas — rounded corners + shadow */}
                  ...
                  {/* Margin note icons — right of page, outside overflow-hidden */}
                  {pageAnns.map(ann => {
                    const c = COLORS.find(x => x.key === ann.color) ?? COLORS[0];
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between px-5 py-4 border-b border-[#e8e0d0]">
                ...
                <button
                  onClick={() => setNotesOpen(false)}
                  className="text-[#9c8870] hover:text-[#3d2f20] w-8 h-8 flex items-center justify-center rounded-xl hover:bg-[#f5f0e8] transition-colors"
                >
                  ✕
                </button>
              </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex justify-end mt-1.5">
                          <button onClick={() => deleteAnnotation(ann.id)} className="text-[10px] opacity-50 hover:opacity-100 hover:text-red-600 transition-colors">
                            Xóa
                          </button>
                        </div>
                      </motion.div>
